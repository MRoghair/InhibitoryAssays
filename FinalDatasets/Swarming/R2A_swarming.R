...
# R2A data 48h 
R2A_48h_Sw <- read.csv("FinalDatasets/Swarming/R2A_48h_Rep1_Swarming.csv", header = TRUE, nrows = 32)
R2A_48h_Sw$Strain_ID <- as.character(R2A_48h_Sw$Strain_ID)
R2A_48h_Sw_pivot <- pivot_longer(R2A_48h_Sw, c(2:33), values_drop_na = FALSE)
R2A_48h_Sw_pivot$Lawn <- as.factor(R2A_48h_Sw_pivot$Strain_ID)
R2A_48h_Sw_pivot$Spot <- as.factor(R2A_48h_Sw_pivot$name)
R2A_48h_Sw_pivot$value <- as.numeric(R2A_48h_Sw_pivot$value)
R2A48h <- rep("R2A-48h", 1024)
R2A_48h_Sw_pivot$MediaTime <- R2A48h
R2A <- rep("R2A", 1024)
R2A_48h_Sw_pivot$Media <- R2A
Time48h <- rep("48h", 1024)
R2A_48h_Sw_pivot$Time <- Time48h
...
# R2A data 96h 
R2A_96h_Sw <- read.csv("FinalDatasets/Swarming/R2A_96h_Rep1_Swarming.csv", header = TRUE, nrows = 32)
R2A_96h_Sw$Strain_ID <- as.character(R2A_96h_Sw$Strain_ID)
R2A_96h_Sw_pivot <- pivot_longer(R2A_96h_Sw, c(2:33), values_drop_na = FALSE)
R2A_96h_Sw_pivot$Lawn <- as.factor(R2A_96h_Sw_pivot$Strain_ID)
R2A_96h_Sw_pivot$Spot <- as.factor(R2A_96h_Sw_pivot$name)
R2A_96h_Sw_pivot$value <- as.numeric(R2A_96h_Sw_pivot$value)
R2A96h <- rep("R2A-96h", 1024)
R2A_96h_Sw_pivot$MediaTime <- R2A96h
R2A <- rep("R2A", 1024)
R2A_96h_Sw_pivot$Media <- R2A
Time96h <- rep("96h", 1024)
R2A_96h_Sw_pivot$Time <- Time96h
...
# R2A + Fe data 48h 
R2A_Fe_48h_Sw <- read.csv("FinalDatasets/Swarming/R2A_Fe_48h_Rep1_Swarming.csv", header = TRUE, nrows = 32)
R2A_Fe_48h_Sw$Strain_ID <- as.character(R2A_Fe_48h_Sw$Strain_ID)
R2A_Fe_48h_Sw_pivot <- pivot_longer(R2A_Fe_48h_Sw, c(2:33), values_drop_na = FALSE)
R2A_Fe_48h_Sw_pivot$Lawn <- as.factor(R2A_Fe_48h_Sw_pivot$Strain_ID)
R2A_Fe_48h_Sw_pivot$Spot <- as.factor(R2A_Fe_48h_Sw_pivot$name)
R2A_Fe_48h_Sw_pivot$value <- as.numeric(R2A_Fe_48h_Sw_pivot$value)
R2AFe48h <- rep("R2AFe-48h", 1024)
R2A_Fe_48h_Sw_pivot$MediaTime <- R2AFe48h
R2AFe <- rep("R2A+Fe", 1024)
R2A_Fe_48h_Sw_pivot$Media <- R2AFe
Time48h <- rep("48h", 1024)
R2A_Fe_48h_Sw_pivot$Time <- Time48h
...
# R2A + Fe data 96h 
R2A_Fe_96h_Sw <- read.csv("FinalDatasets/Swarming/R2A_Fe_96h_Rep1_Swarming.csv", header = TRUE, nrows = 32)
R2A_Fe_96h_Sw$Strain_ID <- as.character(R2A_Fe_96h_Sw$Strain_ID)
R2A_Fe_96h_Sw_pivot <- pivot_longer(R2A_Fe_96h_Sw, c(2:33), values_drop_na = FALSE)
R2A_Fe_96h_Sw_pivot$Lawn <- as.factor(R2A_Fe_96h_Sw_pivot$Strain_ID)
R2A_Fe_96h_Sw_pivot$Spot <- as.factor(R2A_Fe_96h_Sw_pivot$name)
R2A_Fe_96h_Sw_pivot$value <- as.numeric(R2A_Fe_96h_Sw_pivot$value)
R2AFe96h <- rep("R2AFe-96h", 1024)
R2A_Fe_96h_Sw_pivot$MediaTime <- R2AFe96h
R2AFe <- rep("R2A+Fe", 1024)
R2A_Fe_96h_Sw_pivot$Media <- R2AFe
Time96h <- rep("96h", 1024)
R2A_Fe_96h_Sw_pivot$Time <- Time96h
...
R2A_Sw_ALL <- rbind(R2A_48h_Sw_pivot, R2A_96h_Sw_pivot, R2A_Fe_48h_Sw_pivot, R2A_Fe_96h_Sw_pivot)
...
ggplot(data = R2A_Sw_ALL, aes(x=factor(Spot, level = strain_order_names),
                                 y=factor(Lawn, level = strain_order_names), fill=value) ) + 
  facet_grid(Time ~ Media) +
  geom_tile()  + xlab('Spot') + ylab('Lawn') +
  ggtitle("Swarming/Spreading of Spots on R2A") + 
  scale_fill_gradient(low = "whitesmoke", high = "darkslateblue", 
                       space = "Lab",
                       na.value = "darkslateblue", limit = c(0,20)) +
  labs(fill = "") + 
...
Bacillus_only <- c('Bacillus_S18')

Common_Spreaders <- c('Bacillus_S18', 'Variovorax_R27',
                      'Hymenobacter_R03', 'Dyadobacter_E01', 'Chryseobacterium_S02', 'Pedobacter_R06')

ggplot(subset(R2A_Sw_ALL, Spot %in% Bacillus_only), aes(x=factor(Spot, level = strain_order_names),
                           y=factor(Lawn, level =strain_order_names), fill=value) ) + 
  facet_grid(Time ~ Media) +
  geom_tile()  + xlab('Spot') + ylab('Lawn') +
  ggtitle("Bacillus Swarming on R2A") + 
  scale_fill_gradient(low = "grey97", high = "darkslateblue", 
                       space = "Lab",  
                       na.value = "darkslateblue", limit = c(0,20)) +
  labs(fill = "") + 
  theme(axis.text.x = element_text(size=8, angle = 45, vjust = 1, hjust = 1, face="italic", color="black"),
        axis.text.y=element_text(face="italic", size=8, color=strain_name_colors),
        plot.title = element_text(hjust = 0.5, size=16),
        axis.title=element_text(size=12),
        plot.background = element_rect(fill="white"),
