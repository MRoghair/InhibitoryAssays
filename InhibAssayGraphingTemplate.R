################### ggplot tutorial using the Final R2A Dataset ##########################
...
#    - Include my 4 R2A datasets too if you want to make sure you can get them to work before moving on to your own!
...
setwd("~/Desktop/InhibitoryAssays")     #set the path to the location of your data
library(tidyverse)                      #tidyverse is a package we use to make graphs



# R2A data 48h 
R2A_48h <- read.csv("FinalDatasets/R2A_48h_Rep1_FINAL.csv", header = TRUE, nrows = 32)  #import data, include the first row as a header, and cut off anything beyond 32 rows (32 organisms)
R2A_48h$Strain_ID <- as.character(R2A_48h$Strain_ID)                           #I make sure to tell R which of my columns are "characters" vs "numeric"
R2A_48h_pivot <- pivot_longer(R2A_48h, c(2:33), values_drop_na = FALSE)        #this will turn our "square" table into a "long" table. You'll get a column each for the producer, the receiver, and the number 
R2A_48h_pivot$Lawn <- as.factor(R2A_48h_pivot$Strain_ID)                       #This command creates a new column with the same data as Strain_ID, but it is now named "Lawn"
R2A_48h_pivot$Spot <- as.factor(R2A_48h_pivot$name)                            #This command creates a new column with the same data as name, but it is now named "Spot"
R2A_48h_pivot$value <- as.numeric(R2A_48h_pivot$value)                         #Making sure everything in the "value" column is a number. If you get an error, you likely have some text in one of your cells that didn't get deleted
R2A48h <- rep("R2A-48h", 1024)                                                 #Creating a list that repeats "R2A-48h" 1024 times
R2A_48h_pivot$MediaTime <- R2A48h                                              #Appending the list to the dataset and naming the new columns "MediaTime"
R2A <- rep("R2A", 1024)                                                        #Creating a list that repeats "R2A" 1024 times
R2A_48h_pivot$Media <- R2A                                                     #Appending the list to the dataset and naming it "media"
Time48h <- rep("48h", 1024)                                                    #Creating a list that repeats "48h" 1024 times
R2A_48h_pivot$Time <- Time48h                                                  #Appending the list to the dataset and naming it "Time"

# these comments apply to the next 3 groups of commands in the same way


HalfTSA <- read.csv("HannahGraphing/HalfTSA_Fe_48h.csv", header = TRUE, nrows = 32)  #import data, include the first row as a header, and cut off anything beyond 32 rows (32 organisms)
...
ggplot(data = R2A_ALL, aes(x=factor(Spot, level = strain_order_names),                        #pulling data from R2A_All, with "Spot" along the X axis and ordered by the list "strain_order_names"
  ...
  facet_grid(Time ~ Media) +                                                                  #facet_grid splits your plot up by other characteristics you add to your dataset. For example. we run the same analysis on R2A w/ and w/o iron, so we facet by "media" in one direction and "time" in the other
  ...
  ggtitle("Interactions on R2A") +                                                            #Adding a title to the whole plot
...
# If you only want to plot a subset of your data, you can pop this command in place of "R2A_All" 
# Here, we're subsetting it out so we're only looking at Pseudomonas E05's interactions where it is a spot. 
subset(R2A_ALL, Spot %in% c('Pseudomonas_E05'))
...
write_csv(R2A_48h_Cyto_pivot, "R2A_48h_Cyto_pivot.csv")                     # the first value is the name of your dataframe, the second is the name of the thing you're exporting
